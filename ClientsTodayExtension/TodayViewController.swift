...
class TodayExtensionViewController: UIViewController {
    ...
    @IBAction func unoccupiedPlacesButtonPressed(_ sender: UIButton) {
        ...
        UIPasteboard.general.string = (string != "") ? string : NSLocalizedString("UNOCCUPIED_PLACES_WERE_NOT_FOUND", comment: "Свободных мест не найдено")
    }
...
extension TodayExtensionViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        components.queryItems = [
            URLQueryItem(name: "visitId", value: "\(tableData[indexPath.row].objectID.uriRepresentation().absoluteString)")
        ]
...
extension TodayExtensionViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: ReusableViewID.todayExtensionVisitCell, for: indexPath) as! VisitHistoryTableCell
