...
class MonthData {
    ...
    /**
     ...
     Данные по каждому дню извлекаются из БД отдельно только в момент первого обращения к ним и
     сохраняются до удаления контейнера или вызова метода `reset()`
     */
    ...
    fileprivate subscript(index: Int) -> DayData {
...
class CalendarData {
    ...
    init(startYear: Int, numberOfYears: Int) {
        months = []
        for mon in 0 ..< numberOfYears * 12 {
            months.append(MonthData(month: mon % 12 + 1, year: startYear + mon / 12))
        }
    }

    /// Получить данные о календарном дне с номером `indexPath.item` (от 1 до 31)
    /// из секции календаря `indexPath.section`
