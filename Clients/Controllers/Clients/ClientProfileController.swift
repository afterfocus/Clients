...
class ClientProfileController: UIViewController {
    ...
    /// Заголовочное представление списка записей, содержащее номер телефона, ссылку на профиль VK, заметки и фильтр списка записей
...
extension ClientProfileController: SegueHandler {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case .showVisitInfo:
            if let target = segue.destination as? VisitInfoController, let indexPath = visitHistoryTable.indexPathForSelectedRow {
...
extension ClientProfileController: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // Обновить верхний отступ содержимого таблицы на величину не больше максимально допустимой высоты верхнего представления
